...
from data import RAID, THE_PROFESSOR
...
async def dmraid(xspam: Client, message: Message):
      ...
      if len(alt) == 3:
          ...
          id = ok.id
          if id in THE_SHUKLA:
                await message.reply_text(f"ᴠᴇʀɪғɪᴇᴅ ʙʏ ™°‌ 𝐏𝐑𝐎𝐅𝐄𝐒𝐒𝐎𝐑 !")
          ...
          id = ok.id
          if id in THE_SHUKLA:
                await message.reply_text(f"ᴠᴇʀɪғɪᴇᴅ ™°‌ 𝐏𝐑𝐎𝐅𝐄𝐒𝐒𝐎𝐑 !")
          elif id in SUDO_USERS:
                await message.reply_text(f"ᴛʜɪs ᴘᴇʀsᴏɴ ɪs ᴍʏ sᴜᴅᴏ ᴜsᴇʀ !")
          else:
              counts = int(alt[1])
...
async def dmspam(xspam: Client, message: Message):
    ...
    if  len(alt) == 4:
        ...
        if id in THE_SHUKLA:
            await message.reply_text(f"™°‌ 𝐏𝐑𝐎𝐅𝐄𝐒𝐒𝐎𝐑 !")
        elif id in SUDO_USERS:
            await message.reply_text(f"ᴛʜɪs ᴘᴇʀsᴏɴ ɪs ᴍʏ sᴜᴅᴏ ᴜsᴇʀ !")
        else:
            quantity = int(alt[1])
        ...
        await message.reply_text("⚡ ᴜsᴀɢᴇ:\n\n!dmspam 13 <ᴜꜱᴇʀ ɪᴅ> sᴛʀᴀɴɢᴇʀ\n\n !dmspam 13 PROFESSOR PAPA <ʀᴇᴘʟʏ ᴛᴏ ᴜsᴇʀ>")
